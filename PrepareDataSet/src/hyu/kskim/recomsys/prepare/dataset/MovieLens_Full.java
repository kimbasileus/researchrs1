...
public class MovieLens_Full {
	String schema = null;
	String path = "D:\\Research_LibraryDataSet\\Dataset\\MovieLens_middle"; // Dataset directory path
	String dbId = "root";
	...
	public MovieLens_Full(String schema, String dataset_Path, int numOfUsers, int numOfItems, int numOfRatings) {
		this.db.connectDB("root", "kyungsookim");
		
		if(schema!=null) this.schema = schema;
		if(dataset_Path!=null) this.path = dataset_Path;
		this.numOfUsers = numOfUsers; // 138493
		this.numOfItems = numOfItems; // 27278 
		this.numOfRatings = numOfRatings; // 20000263 
	}
	...
	//	this.loadRatings_into_DB(this.schema); // 위의 changeItemID 함수와 꼭 함께 실행해야 한다.	
	//	this.loadUsers_into_DB(this.schema);
	}
	
	// 실행 #1. Load user-item rating into DB
	public void loadRatings_into_DB(String schema) {  // If the table is trainset, tableName=ratings_trainset
		...
		try{
			this.db.getStmt().execute("delete FROM "+schema+".ratings;");
			
			BufferedReader reader = new BufferedReader(new FileReader(dir));
			String inputLine = null;
			 
			String temp[] = null; int userID; int movieLensID; double rating; int timestamp; int newID;
			int count = -1;
			while ((inputLine = reader.readLine()) != null){
				if(count==-1) {
				 ...
				 movieLensID = Integer.parseInt(temp[1]);
				 rating = Double.parseDouble(temp[2]);
				 timestamp = Integer.parseInt(temp[3]);
				 newID = this.map.get(movieLensID);
				 
				 
				 count++;
				 System.out.println("["+count+"] \t"+userID+"\t"+newID+"\t"+movieLensID+"\t"+rating+"\t"+timestamp);
				 
				 String sql = null;
			     sql = "INSERT INTO `"+schema+"`.`ratings` (`ID`, `userID`, `itemID`, `movielensID`, `rating`, `timestamp`) "
							+ "VALUES ('"+count+"', '"+userID+"', '"+newID+"', '"+movieLensID+"', '"+rating+"', '"+timestamp+"');";
	...
	// 실행 #2. Load User's ID and their name into the DB.
	public void loadUsers_into_DB(String schema) {
		try {
			this.db.getStmt().execute("delete FROM "+schema+".users;");
			
			ArrayList<Integer> userList = new ArrayList<Integer>();
			String sql = "SELECT distinct userID FROM "+schema+".ratings order by userID asc;";
	...
	// 실행 #3. Load all item information into the DB.
	public void loadItems_into_DB(String schema) {
		String dir = path+"\\movies.csv";
		try{
			 this.db.getStmt().execute("delete FROM "+schema+".items;");
	...
	// 실행 #4. Load all movie URL data into the DB.
	...
	// 실행 #5. 모든 아이템들에 대하여 기존 MovieLensID를 신규 아이템 ID로 재 배정한다.
	...
	/////// Part 2. Make Traing & Test DataSet for Experiments
	
	// 실행 #1. Make Test Dataset from full rating matrix. It ouputs text file (rating IDs)
	...
	// 실행 #3. 누적 테스트셋이 올바르게 생성되었는지 중복성 여부 검증 단계
	...
	public void loadTrain_Test_Dataset_Into_DB(String fileDir, String schema, double testsetRatio) {
		try {
			...
			// 3. Delete testset from training set and Write testset onto the testset DB
				...
				this.db.getStmt().executeUpdate("INSERT INTO `"+schema+"`.`ratings_testset` (`ID`, `userID`, `itemID`, `movielensID`, `actual_rating`, `timestamp`) "
	...
	// 실행 #5. Training Set에 기반하여 사용자별 평점 평균들을 계산하여 DB에 캐싱하는 함수
	...
	// 실행 #6. 모든 rating, training, test set에 대하여 Item ID 최종 검증
